...
from agentverse_client.search.aio import AgentFilters, SortType, Direction, AgentSearchResponse
...
class AsyncAIEngine:
    ...
    def __init__(self, url: str = "https://agentverse.ai"):
        self.configuration = agentverse_client.search.Configuration(
            host=url
        )
    ...
    async def search_agent(
        ...
        async with agentverse_client.search.aio.ApiClient(self.configuration) as api_client:
            ...
            try:
                # Search Agents
                api_response: AgentSearchResponse = await api_instance.search_agents(agent_search_request)
                return api_response
